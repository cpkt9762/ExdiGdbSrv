...
HRESULT STDMETHODCALLTYPE CLiveExdiGdbSrvServer::DoSingleStep(DWORD dwProcessorNumber)
{
	try
	{
		...
		{
			printf("DoSingleStep: Invalid processor number %d\n", dwProcessorNumber);
			return E_INVALIDARG;
		}
		AsynchronousGdbSrvController* pController = GetGdbSrvController();
		if (pController == nullptr)
	...
	/* [out] */ IeXdiCodeBreakpoint3** ppieXdiCodeBreakpoint)
{
	...
	*ppieXdiCodeBreakpoint = nullptr;
	...
	if (mt != mtVirtual)
	{
		printf("AddCodeBreakpoint: Invalid memory type %d\n", mt);
		return E_INVALIDARG;
	}
	...
	/* [out] */ IeXdiDataBreakpoint3** ppieXdiDataBreakpoint)
{
	...
	*ppieXdiDataBreakpoint = nullptr;
	...
	if (mt != mtVirtual)
	{
		printf("AddDataBreakpoint: Invalid memory type %d\n", mt);
		return E_INVALIDARG;
	}
	...
	/* [in] */ DWORD dwConnectionCookie)
{
	...
	{
		printf("StopNotifyingRunChg: Invalid connection cookie %d\n", dwConnectionCookie);
		return E_INVALIDARG;
	}
	...
	/* [out] */ DWORD* pdwBytesWritten)
{
	try
	{
		...
		{
			printf("WriteVirtualMemory: Invalid arguments\n");
			return E_POINTER;
		}

		if (pBuffer->cDims != 1)
		{
			printf("WriteVirtualMemory: Invalid number of dimensions %d\n", pBuffer->cDims);
			return E_INVALIDARG;
		}
		...
		{
			printf("WriteVirtualMemory: Invalid data type %d\n", dataType);
			return E_INVALIDARG;
		}
	...
	/* [out] */ DWORD* pdwBytesWritten)
{
	...
	try
	{
		...
		if (pBuffer == nullptr || pdwBytesWritten == nullptr || pController == nullptr)
		{
			printf("WritePhysicalMemoryOrPeriphIO: Invalid arguments\n");
			return E_POINTER;
		}

		if (pBuffer->cDims != 1)
		{
			printf("WritePhysicalMemoryOrPeriphIO: Invalid number of dimensions %d\n", pBuffer->cDims);
			return E_INVALIDARG;
		}
	...
	/* [out] */ SAFEARRAY** pOutputBuffer)
{
	...
	try
	{
		...
		{
			printf("DBGENG_EXDI_IOCTL_V3_GET_NT_BASE_ADDRESS_VALUE:  found nt\n");
			static  uint64_t ntosbase = 0;

			if (ntosbase)
			{
				return SafeArrayFromByteArray(reinterpret_cast<const char*>(&ntosbase), sizeof(DWORD64), pOutputBuffer);
			}

			auto  ruskt = QRcmdRegistorSegment(pController, "r idtr");

			if (!ruskt.first)
			{
				printf("GetContextEx Faild IDTBase:%p %x\n", ruskt.first, ruskt.second);
				hr = E_NOTIMPL;
				MessageBoxA(nullptr, "The NT base address was not found.", nullptr, MB_ICONERROR);
				return hr;
			}

			memoryAccessType memType = { 0 };
			pController->GetMemoryPacketType(m_lastPSRvalue, &memType);
			SimpleCharBuffer buffer = pController->ReadMemoryEx(ruskt.first, 16, memType);

			if (buffer.IsEmpty())
			{
				printf("GetContextEx Faild IDTBase:%p %x\n", ruskt.first, ruskt.second);
				hr = E_NOTIMPL;
				MessageBoxA(nullptr, "The1 NT base address was not found.", nullptr, MB_ICONERROR);
				return hr;
			}
			...
			printf("kiDivide = %I64X\n", kiDivide);
			SimpleCharBuffer buffer3 = pController->ReadMemoryEx(kiDivide, 8, memType);

			if (!buffer3.IsEmpty() &&
				((0xec81485508ec8348 != *(DWORD64*)buffer3.GetInternalBuffer())
					&& (0x082444f6 != *(DWORD32*)buffer3.GetInternalBuffer())
					))
			{
				printf("*(DWORD32*)buffer3.GetInternalBuffer():%p\n", *(DWORD32*)buffer3.GetInternalBuffer());
				hr = E_NOTIMPL;
				MessageBoxA(nullptr, "The2 NT base address was not found3.", nullptr, MB_ICONERROR);
				return hr;
			}

			DWORD64 searchBased = kiDivide & 0xFFFFFFFFFFF00000;

			//fffff8011f200000
			//fffff8011ee00000
			//fffff80357600000 
			//FFFFF80366400000
			//kiDivide = FFFFF8066441DE00
		  //0xfffff806`64000000
		  //0xfffff80664000000
		  //FFFFF803 7221DE00
		  //fffff80371e00000
		  ...
		  //fffff80371e00000
			...
			for (size_t i = 0; i < 10; i++)
			{
				...
				try
				{
					...
					printf("searchBased = %I64X\n", searchBased);

					SimpleCharBuffer buffer2 = pController->ReadMemoryEx(searchBased, 2, memType);
					if (!buffer2.IsEmpty())
					{

						if (0x4D == buffer2[0] && 0x5A == buffer2[1])
						{
							ntosbase = searchBased;
							break;
						}
					}
					searchBased -= 0x200000;

				}
				catch (_com_error const& error)
				{
					printf("GetContextEx Faild IDTBase:%p %x %x\n", ruskt.first, ruskt.second, error.Error());
					hr = E_NOTIMPL;
				...
				{
					printf("GetContextEx Faild IDTBase:%p %x\n", ruskt.first, ruskt.second);
					MessageBoxA(nullptr, "ReadMemory The NT base address was not found1.", nullptr, MB_ICONERROR);
					hr = E_NOTIMPL;
				}
			}
			...
			if (!ntosbase)
			{
				MessageBoxA(nullptr, "ntosbase The NT base address was not found3.", nullptr, MB_ICONERROR);
				hr = E_NOTIMPL;
			}
			else
			{
				hr = SafeArrayFromByteArray(reinterpret_cast<const char*>(&ntosbase), sizeof(DWORD64), pOutputBuffer);
	...
	/* [out] */ ULONG64* pKPCRPointer)
{
	...
	try
	{
		...
		{
			printf("GetKPCRForProcessor: Invalid arguments\n");
			return E_POINTER;
		}
		...
		if (FAILED(result))
		{
			printf("GetKPCRForProcessor: GetNumberOfProcessors failed with %x\n", result);
			return result;
		}
		if (dwProcessorNumber >= totalProcessors)
		{
			printf("GetKPCRForProcessor: Invalid processor number %d\n", dwProcessorNumber);
			return E_INVALIDARG;
		}
		*pKPCRPointer = pController->GetKpcrOffset(dwProcessorNumber);
		if (*pKPCRPointer == 0)
		{
			printf("GetKPCRForProcessor: KPCR pointer is not set for processor %d\n", dwProcessorNumber);
			return E_NOTIMPL;
		}
		return S_OK;
	}
	...
	/* [out] */ SAFEARRAY** pKdVersionBlockBuffer)
{
	...
	printf("ReadKdVersionBlock: Not implemented\n");
	return E_NOTIMPL;
}
	...
	/* [out] */ DWORD* pdwNumberOfProcessors)
{
	...
	try
	{
		...
		if (pController == nullptr)
		{
			printf("GetNumberOfProcessors: Invalid arguments\n");
			return E_POINTER;
		}
...
HRESULT STDMETHODCALLTYPE CLiveExdiGdbSrvServer::GetContextEx(_In_ DWORD processorNumber, _Inout_ PCONTEXT_X86_64 pContext)
{
	...
	printf("GetContextEx processorNumber:%d %d %d %d\n", processorNumber, i, m_targetIsRunning, GetTickCount() - LastTime);
	i++;

	if (pContext == nullptr)
	{
		printf("GetContextEx1 pController == nullptr\n");
		return E_POINTER;
	}
	auto iter = m_cachecontextsMap.find(processorNumber);
	if (iter != m_cachecontextsMap.end())
	{
		memcpy(pContext, &iter->second, sizeof(CONTEXT_X86_64));
		return S_OK;
	}

	/*if ((GetTickCount() - LastTime)<200)
	{
		auto iter = m_cachecontextsMap.find(processorNumber);
		if (iter != m_cachecontextsMap.end())
		{
	...
	try
	{
		...
		if (pController == nullptr)
		{
			printf("GetContextEx2 pController == nullptr\n");
			return E_POINTER;
		}
		pController->StopTargetAtRun();
		...
		assert(!registers["cr0"].empty());
		assert(!registers["cr2"].empty());
		assert(!registers["cr3"].empty());
		assert(!registers["cr4"].empty());
		assert(!registers["cr8"].empty());
		...
		if (registers.find("dr0") != registers.end())
		{
			pContext->Dr0 = static_cast<DWORD64>(GdbSrvController::ParseRegisterValue(registers["dr0"]));
			pContext->Dr1 = static_cast<DWORD64>(GdbSrvController::ParseRegisterValue(registers["dr1"]));
			pContext->Dr2 = static_cast<DWORD64>(GdbSrvController::ParseRegisterValue(registers["dr2"]));
			pContext->Dr3 = static_cast<DWORD64>(GdbSrvController::ParseRegisterValue(registers["dr3"]));
			pContext->Dr6 = static_cast<DWORD64>(GdbSrvController::ParseRegisterValue(registers["dr6"]));
			pContext->Dr7 = static_cast<DWORD64>(GdbSrvController::ParseRegisterValue(registers["dr7"]));
		...
		if (pContext->GDTBase == 0)
		{
			printf("GetContextEx pContext->GDTBase==%xx\n", registers["gdtrbase"]);

			assert(pContext->GDTBase != 0);
		}
	  ...
	  /*  printf("Rax:%llx\n", pContext->Rax);
		...
		printf("RegCr4:%llx\n", pContext->RegCr4);
		printf("RegCr8:%llx\n", pContext->RegCr8);
		printf("ControlWord:%llx\n", pContext->ControlWord);
		printf("StatusWord:%llx\n", pContext->StatusWord);
		printf("TagWord:%llx\n", pContext->TagWord);
		printf("ErrorOffset:%llx\n", pContext->ErrorOffset);
		printf("ErrorSelector:%llx\n", pContext->ErrorSelector);
		printf("DataOffset:%llx\n", pContext->DataOffset);
		printf("DataSelector:%llx\n", pContext->DataSelector);
		printf("GDTBase:%llx\n", pContext->GDTBase);
		printf("GDTLimit:%llx\n", pContext->GDTLimit);
		printf("IDTBase:%llx\n", pContext->IDTBase);
		printf("IDTLimit:%llx\n", pContext->IDTLimit);
		...
		m_cachecontextsMap[processorNumber] = *pContext;
	...
	/* [in] */ LPCWSTR pFunctionToExecute)
{
	try
	{
		...
		{
			printf("ExecuteExdiComponentFunction: pFunctionToExecute is null\n");
			return E_POINTER;
		}
		if (type != exdiComponentSession)
		{
			printf("ExecuteExdiComponentFunction: type is not exdiComponentSession\n");
			return E_INVALIDARG;
		}
		AsynchronousGdbSrvController* pController = GetGdbSrvController();
		if (pController == nullptr)
		{
			printf("ExecuteExdiComponentFunction: pController is null\n");
			return E_POINTER;
		}
		if (!pController->ExecuteExdiFunction(dwProcessorNumber, pFunctionToExecute))
		{
			printf("ExecuteExdiComponentFunction: ExecuteExdiFunction failed\n");
			return E_FAIL;
		}
		return S_OK;
	}
