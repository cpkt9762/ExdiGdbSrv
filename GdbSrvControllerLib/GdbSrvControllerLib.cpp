...
public:
	...
	SimpleCharBuffer GdbSrvControllerImpl::ExecuteExdiGdbSrvMonitor(_In_ unsigned core, _In_ LPCWSTR pCmdToExecute)
	{
		...
		if (m_pRspClient->GetRspSessionStatus(gdbServerError, core))
		{
			if (gdbServerError != ERROR_SUCCESS)
			{
				...
				{
					printf("Failed to attach to GdbServer on core %d\n", core);
		...
		{
			printf("Failed to execute monitor command %s\n", pMonitorCmd);
			throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
		}
	...
	bool GdbSrvControllerImpl::ExecuteExdiFunction(_In_ unsigned core, _In_ LPCWSTR pFunctionToExecute)
	{
		...
		if (itFunction == m_exdiFunctions.end())
		{
			...
			throw _COM_ERROR_EXCEPTION_HELPER_(E_NOTIMPL);
		}
	...
	TARGET_HALTED GdbSrvControllerImpl::ReportReasonTargetHalted(_Out_ StopReplyPacketStruct* pStopReply)
	{
		...
		printf("ReportReasonTargetHalted Send the ? packet\n");
		printf("ReportReasonTargetHalted Send the ? packet\n");
		printf("ReportReasonTargetHalted Send the ? packet\n");
		//  Send the "?" packet    
	...
	{
		printf("GdbSrvControllerImpl::ExecuteCommandOnProcessor: %s isRspWaitNeeded:%d\n", pCommand, isRspWaitNeeded); 
		std::lock_guard<std::mutex> lock(__lock);
		if (pCommand == nullptr)
		{
			printf("GdbSrvControllerImpl::ExecuteCommandOnProcessor: Invalid command pointer\n");
		...
		{
			printf("GdbSrvControllerImpl::ExecuteCommandOnProcessor: Invalid string size %d %d %s\n", result.max_size(), stringSize, result.c_str());
			throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
		}
		...
		//            printf("GdbSrvControllerImpl::ExecuteCommandOnProcessor: Error %d\n", m_pRspClient->GetRspLastError());
		//            throw _COM_ERROR_EXCEPTION_HELPER_(HRESULT_FROM_WIN32(m_pRspClient->GetRspLastError()));
		//        }
		//    }
		//}
		bool IsPollingChannelMode = false;
		bool isDone = m_pRspClient->SendRspPacketEx(command, processor, result, isRspWaitNeeded, IsPollingChannelMode, true);
		if (!isDone)
		{
			//  Did the user interrupt?
			if (!m_pRspClient->GetInterruptFlag())
	...
	{
		printf(__FUNCTION__" stringSize:%d processor:%d\n", stringSize, processor);

		std::string result;
		if (result.max_size() < stringSize)
		{
			printf("GdbSrvControllerImpl::GetResponseOnProcessor: Invalid string size %d %d %s\n", result.max_size(), stringSize, result.c_str());
			throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
		}
	...
	std::string GdbSrvControllerImpl::ExecuteCommandOnMultiProcessors(_In_ LPCSTR pCommand, _In_ bool isRspWaitNeeded, _In_ size_t stringSize)
	{
		...
		bool isDone = false;
		unsigned numberOfCoreConnections = static_cast<unsigned>(m_pRspClient->GetNumberOfStreamConnections());
		printf(__FUNCTION__" numberOfCoreConnections:%d\n", numberOfCoreConnections);
		for (unsigned core = 0; core < numberOfCoreConnections; ++core)
		{
			isDone = m_pRspClient->SendRspPacket(command, core);
			printf(__FUNCTION__" numberOfCoreConnections:%d isDone:%d\n", numberOfCoreConnections, isDone);
			if (!isDone)
			{
				break;
			}
	...
	static ULONGLONG GdbSrvControllerImpl::ParseRegisterValue(_In_ const std::string& stringValue)
	{
		...
		if (sscanf_s(stringValue.c_str(), "%llx", &result.QuadPart) != 1)
		{
			printf("GdbSrvControllerImpl::ParseRegisterValue: Invalid register value %s\n", stringValue.c_str());
			//throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
			return 0;
		}
	...
	static DWORD GdbSrvControllerImpl::ParseRegisterValue32(_In_ const std::string& stringValue)
	{
		...
		{
			printf("GdbSrvControllerImpl::ParseRegisterValue32: Invalid register value %s\n", stringValue.c_str());
			throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
		}
		return result;
	}
	...
	//      慐 NN?             Error reading the registers.
	//
	//  Example:
		...
		_In_ RegisterGroupType groupType = CORE_REGS)
	{
		...
		{
			printf("GdbSrvControllerImpl::QueryAllRegistersEx: Failed to set the processor core %d\n", processorNumber);
			throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
		}
		...
		if (IsReplyError(reply))
		{
			printf("GdbSrvControllerImpl::QueryAllRegistersEx: Failed to get the registers %s\n", reply.c_str());
			throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
		}
			...
			++it)
		{
			...
			// printf(__FUNCTION__" %s %s\n", it->name.c_str(),result[it->name].c_str());
			startIdx += endIdx;
		}
		return result;
	}
		...
		_In_ RegisterGroupType groupType = CORE_REGS)
	{
		if (processorNumber != -1)
		{
			...
			if (!SetThreadCommand(processorNumber, "g"))
			{
				printf("GdbSrvControllerImpl::SetRegistersEx: Failed to set the processor core %d\n", processorNumber);
				throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
			}
		}
		for (auto const& kv : registerValues)
		{
			...
			if (!IsReplyOK(reply))
			{
				printf("GdbSrvControllerImpl::SetRegistersEx: Failed to set the register %s\n", reply.c_str());
				throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
			}
		}
	}
	...
	//
	//  Request:
	//  憄 n?              Reads the register n (where the register number n is in hexadecimal ascii number).
	//               
	//  Response:
	//  慩X厭               Success.
	//  慐 NN?             Error.
		...
		_In_ RegisterGroupType groupType = CORE_REGS)
	{
		if (processorNumber != -1)
		{
			...
			if (!SetThreadCommand(processorNumber, "g"))
			{
				printf("GdbSrvControllerImpl::QueryRegistersEx: Failed to set the processor core %d\n", processorNumber);
				throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
			}
		}
		...
		for (size_t index = 0; index < numberOfElements; ++index)
		{
			...
			{
				printf("GdbSrvControllerImpl::QueryRegistersEx: Failed to get the register %s\n", reply.c_str());
				throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
			}
		...
		_Out_ int& maxRegisterNameLength)
	{
		if (processorNumber != -1)
		{
			...
			if (!SetThreadCommand(processorNumber, "g"))
			{
				printf("GdbSrvControllerImpl::QueryRegistersByGroup: Failed to set the processor core %d\n", processorNumber);
				throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
			}
		}
			...
			it != RegistersEnd(groupType); ++it)
		{
			...
			if (IsReplyError(reply) || reply.empty())
			{
				printf("GdbSrvControllerImpl::QueryRegistersByGroup: Failed to get the register %s\n", reply.c_str());
				throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
			}
	...
	{
		printf(__FUNCTION__" systemRegIndex:%d maxSize:%d memType:%d\n", systemRegIndex, maxSize, memType);
		...
		{
			printf("GdbSrvControllerImpl::ReadSystemRegistersFromGdbMonitor: Invalid system register index %d\n", systemRegIndex);
			throw _COM_ERROR_EXCEPTION_HELPER_(E_NOTIMPL);
		}
		...
		{
			printf("GdbSrvControllerImpl::ReadSystemRegistersFromGdbMonitor: Invalid memory value size %d\n", strlen(pFormat) + 1);
			throw _COM_ERROR_EXCEPTION_HELPER_(E_OUTOFMEMORY);
		}
		...
		{
			printf("GdbSrvControllerImpl::ReadSystemRegistersFromGdbMonitor: Invalid response %s\n", memoryValueStr.c_str());
			throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
		}
		...
		{
			printf("GdbSrvControllerImpl::ReadSystemRegistersFromGdbMonitor: Invalid register value %s\n", memoryValueStr.c_str());
			throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
		}
		...
		_In_ const memoryAccessType memType)
	{
		...
		printf("systemRegMapResult[arraySystemRegisterToQuery[0]]:%s\n", systemRegMapResult[arraySystemRegisterToQuery[0]].c_str());
		...
		_In_ bool fReportWriteError)
	{
		...
		{
			printf("GdbSrvControllerImpl::WriteSystemRegisters: Unsupported function\n");
			//  Unsupported function
			throw _COM_ERROR_EXCEPTION_HELPER_(E_NOTIMPL);
		}
	...
	SimpleCharBuffer GdbSrvControllerImpl::ReadMemory(_In_ AddressType address, _In_ size_t maxSize,
		_In_ const memoryAccessType memType)
	{
		//printf(__FUNCTION__" address:%p maxSize:%d memType:%d\n", address, maxSize, memType);
		//
		assert(static_cast<int>(maxSize) > 0);
		if (address < 0x1000)
		{
			printf("GdbSrvControllerImpl::ReadMemory: address %I64x maxSize %d memType %d\n", address, maxSize, memType);
			throw _COM_ERROR_EXCEPTION_HELPER_(E_OUTOFMEMORY);
		}
		assert(address != 0);
		...
		{
			printf("GdbSrvControllerImpl::ReadMemory: Failed to allocate memory for the result\n");
			throw _COM_ERROR_EXCEPTION_HELPER_(E_OUTOFMEMORY);
		}
		...
		while (maxSize != 0)
		{
			...
			for (;;)
			{
				...
				{
					printf("GdbSrvControllerImpl::ReadMemory:address:%p size:%x memoryCmd %s\n", address, size, memoryCmd);
				...
				if (messageLength > maxReplyLength)
				{

					printf("GdbSrvControllerImpl::ReadMemory: messageLength %d maxReplyLength %d maxPacketLength:%d requestSize:%d\n", messageLength, maxReplyLength, maxPacketLength, requestSize);
					assert(messageLength < maxReplyLength);
				}
				...
				if (messageLength == 0 && result.GetLength() == 0)
				{
					...
					{
						printf("GdbSrvControllerImpl::ReadMemory: Failed to read memory %p %s\n", address, reply.c_str());
						//  Yes, it is unacceptable
						throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
					}
					printf("GdbSrvControllerImpl::ReadMemory: Failed1 to read memory %p %s\n", address, reply.c_str());
					printf("GdbSrvControllerImpl::ReadMemory: address %I64x maxSize %d memType %d\n", address, maxSize, memType);
					fError = true;
					assert(!fError);
					break;
				...
				if (IsReplyError(reply))
				{
					...
					printf("GdbSrvControllerImpl::ReadMemory: Failed2 to read memory %p %s\n", address, reply.c_str());
					//assert(!fError);
					break;
				}
				...
				for (size_t pos = 0; pos < messageLength; pos += 2)
				{
					...
					// printf("GdbSrvControllerImpl::ReadMemory: oneByteMemory %s\n", oneByteMemory.c_str());
					int value = 0;
					if (sscanf_s(oneByteMemory.c_str(), "%x", &value) != 1)
					{
						printf("GdbSrvControllerImpl::ReadMemory: Failed to read memory %p size:%x %s\n", address, size, reply.c_str());
						//assert(0);
						throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
					}
					if (maxReplyLength < result.GetLength() || static_cast<int>(size) <= 0)
					{

						printf("GdbSrvControllerImpl::ReadMemory: address %I64x Size %d recvLength %d messageLength:%d\n", address, size, recvLength, messageLength);
						printf("GdbSrvControllerImpl::ReadMemory: address %I64x maxSize %d memType %d\n", address, maxSize, memType);
						printf("GdbSrvControllerImpl::ReadMemory: Failed1 to read memory %d %d\n", maxReplyLength, result.GetLength());
						printf("GdbSrvControllerImpl::ReadMemory: value %x %d\n", value, result.GetLength());
						assert(maxReplyLength > result.GetLength());
					}
					...
					recvLength++;
					size--;
					//00de1000008ee17407f8ffff00000000
					if (static_cast<int>(size) <= 0)
					{
						//printf("GdbSrvControllerImpl::ReadMemory:  address:%p %d %s\n", address, size, reply.c_str());
						assert(static_cast<int>(size) >= 0);
						break;
					}
				}
			...
			if (fError)
			{
				printf("GdbSrvControllerImpl::ReadMemory: Failed5 to read memory \n");
				// assert(!fError);
				throw _COM_ERROR_EXCEPTION_HELPER_(E_OUTOFMEMORY);
				break;
			}
	...
	//  Response: 
	//  慜K?           Success.
	//  慐 NN?         Error (includes the case where only part of the data was written). 
	//
	//  Example:
	//  Request:
	...
	//  for subsequent requests is used 憅sThreadInfo? 
	//  'qfThreadInfo'
	//  'qsThreadInfo'
	//  
	//  Response
	...
	unsigned GdbSrvControllerImpl::GetProcessorCount()
	{
		...
		if (m_cachedProcessorCount == 0)
		{
			...
			if (numberOfCoreConnections == 1)
			{
				...
				if (reply.empty())
				{
					printf("GdbSrvControllerImpl::GetProcessorCount: Failed to get the number of processor cores %s\n", reply.c_str());
					throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
				}
		...
		if (pos != string::npos)
		{
			...
			if (pcEndPos != string::npos)
			{
				...
				if (!pcAddress.empty())
				{
					...
					{
						printf("GdbSrvControllerImpl::FindPcAddressFromStopReply: pcAddress %s %s\n", pcAddress.c_str(), TargetArchitectureHelpers::ReverseRegValue(pcAddress).c_str());
	...
	DWORD GdbSrvControllerImpl::GetProcessorNumberByThreadId(_In_ const std::string& threadId)
	{
		...
		{
			printf("GdbSrvControllerImpl::GetProcessorNumberByThreadId: Invalid threadId\n");
			throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
		}
	...
	inline RSP_Response_Packet GdbSrvControllerImpl::GetRspResponse(_In_ const std::string& reply)
	{
		...
		//  Is it an OK response
	/*	else if (reply[0] == 'E' && reply.length() > 0)
		{
			response = RSP_OK;
		}*/
		...
		_In_ size_t numberOfRegArrayElem)
	{
		...
		if (it == RegistersEnd())
		{
			printf("GdbSrvControllerImpl::CreateNeonRegisterNameArray: Invalid register name %s\n", registerName.c_str());
			throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
		}
	...
	HRESULT GdbSrvControllerImpl::ReadMsrRegister(_In_ DWORD dwProcessorNumber, _In_ DWORD dwRegisterIndex, _Out_ ULONG64* pValue)
	{
		...
		if (statusRegister[0] != nullptr)
		{
			...
			printf("GdbSrvControllerImpl::ReadMsrRegister: statusRegister %x %s\n", statusRegister[0], cpsrRegisterValue[statusRegister[0]].c_str());
	...
	HRESULT GdbSrvControllerImpl::WriteMsrRegister(_In_ DWORD dwProcessorNumber, _In_ DWORD dwRegisterIndex, _In_ ULONG64 value)
	{
		...
		{
			std::map<std::string, std::string> cpsrRegisterValue = QueryRegisters(dwProcessorNumber, statusRegister, ARRAYSIZE(statusRegister));
			printf("GdbSrvControllerImpl::WriteMsrRegister: statusRegister %x %s\n", statusRegister[0], cpsrRegisterValue[statusRegister[0]]);
			ULONGLONG processorStatusRegValue = ParseRegisterValue(cpsrRegisterValue[statusRegister[0]]);
	...
	void GdbSrvControllerImpl::SetSystemRegisterXmlFile(_In_z_ PCWSTR pSystemRegFilePath)
	{
		...
		{
			printf("GdbSrvControllerImpl::SetSystemRegisterXmlFile: Invalid system register xml file path\n");
			throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
		}
		...
		{
			printf("GdbSrvControllerImpl::SetSystemRegisterXmlFile: Failed to allocate memory for system register xml file path\n");
			throw _COM_ERROR_EXCEPTION_HELPER_(E_OUTOFMEMORY);
		}
...
private:
	...
	inline AddressType GetAccessCodeByRegisterNumber(_In_ const string& regOrder)
	{
		...
		{
			printf("GdbSrvControllerImpl::GetAccessCodeByRegisterNumber: Invalid system register access code map\n");
			throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
		}
	...
	inline const char* GetSystemRegNamebyAccessCode(_In_ AddressType regAccess)
	{
		if (m_spSystemRegAccessCodeMap->empty())
		{
			printf("GdbSrvControllerImpl::GetSystemRegNamebyAccessCode: Invalid system register access code map\n");
			throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
		}
	...
	SimpleCharBuffer GdbSrvControllerImpl::PrintRegistersGroup(_In_ RegisterGroupType groupType, _In_ bool verbose = false)
	{
		...
		{
			printf("GdbSrvControllerImpl::PrintRegistersGroup: Failed to allocate memory for monitor result\n");
			throw _COM_ERROR_EXCEPTION_HELPER_(E_OUTOFMEMORY);
		}
			...
			it != RegistersEnd(groupType); ++it, numberOfRegs++)
		{
			if (verbose)
			{
				...
				printf("1GdbSrvControllerImpl::PrintRegistersGroup: Failed to print register %s %s\n", it->name.c_str(), registers[it->name].c_str());
				operationResult = sprintf_s(&regPrtBuffer[index], ARRAYSIZE(regPrtBuffer) - index,
			...
			{
				printf("2GdbSrvControllerImpl::PrintRegistersGroup: Failed to print register %s %s\n", it->name.c_str(), registers[it->name].c_str());
				operationResult = sprintf_s(&regPrtBuffer[index], ARRAYSIZE(regPrtBuffer) - index,
			...
			{
				printf("GdbSrvControllerImpl::PrintRegistersGroup: Failed to print register %s\n", it->name.c_str());
				throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
			}
			index += operationResult;
		...
		_In_ RegisterGroupType regGroup)
	{
		...
		printf("GdbSrvControllerImpl::FindRegisterVectorEntryEx: Invalid register name\n");
		throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
	}
	...
	bool CheckProcessorCoreNumber(_In_ unsigned core)
	{
		...
		if (isAllCores)
		{
			...
			{
				throw _COM_ERROR_EXCEPTION_HELPER_(E_ABORT);
			}
		}
		else
		{
		...
		_In_ size_t lengthArrayElem)
	{
		...
		{
			printf("GdbSrvControllerImpl::FillNeonRegisterNameArrayEntry: Failed to copy register name %s\n", regIt->name.c_str());
			throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
		}
	}
		...
		_In_ const char* lengthToRead)
	{
		...
		do
		{
			...
			{
				printf("GdbSrvControllerImpl::HandleTargetDescriptionPacket: Failed to read target description file\n");
				throw _COM_ERROR_EXCEPTION_HELPER_(E_FAIL);
			}
	...
	void ValidateTargetArchitecture(_Inout_ ConfigExdiGdbServerHelper& cfgData)
	{
		...
		if (cfgData.GetRegisterGroupArchitecture() != cfgData.GetTargetArchitecture())
		{
			...
			{
				printf("GdbSrvControllerImpl::HandleTargetDescriptionPacket: Invalid target architecture\n");
				throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
			}
		}
	...
	void GdbSrvControllerImpl::HandleTargetDescriptionPacket(_In_ ConfigExdiGdbServerHelper& cfgData)
	{
		...
		if (cfgData.IsRegisterGroupFileAvailable(SYSTEM_REGS))
		{
			...
			if (wFileName.empty())
			{
				printf("GdbSrvControllerImpl::HandleTargetDescriptionPacket: Invalid system register file name\n");
				throw _COM_ERROR_EXCEPTION_HELPER_(E_INVALIDARG);
			}
...
{
	//printf(__FUNCTION__" stringValue %s\n", stringValue.c_str());
...
SimpleCharBuffer GdbSrvController::ReadMemoryEx(_In_ AddressType address, _In_ size_t size, _In_ const memoryAccessType memType)
{
	assert(m_pGdbSrvControllerImpl != nullptr);
	try {
